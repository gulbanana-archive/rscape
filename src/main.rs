...
fn _main() -> Result<(), ScapeError> {
    ...
    let mut glyph_cache: HashMap<i32, Texture> = HashMap::new();
    ...
    while !should_quit {
        ...
        if let Err(e) = frame(&mut renderer, &arial128, &mut glyph_cache, i) {
...
fn frame(renderer: &mut Renderer, font: &Font, cache: &mut HashMap<i32, Texture>, i: i32) -> Result<(), ScapeError> {
    if !cache.contains_key(&i) {
        ...
        let new_texture = try!(renderer.create_texture_from_surface(&surface));
        cache.insert(i, new_texture);
    }
    
    let texture = cache.get_mut(&i).unwrap();
